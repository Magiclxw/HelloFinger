...
{
    if(times >4 || id > FINGER_MAX_NUM)
    {
        return  OPERATE_ERROR_INVALID_PARAMETERS;
    }
    uint8_t cmd[8] = {USB_PROTOCOL_FORMAT_ENROLL_FINGER,0x00,0x00,0x00,id,times,param[0],param[1]};
    GenerateCmd(cmd,8);
...
int HID_Send_Account_Password(hid_device *usb_handle,QString account,QString password,uint8_t index)
{
    ...
    delete [] cmd_pack2;
    hid_write(usb_handle,hid_command,account_len+10);
    ...
    delete [] cmd_pack3;
    hid_write(usb_handle,hid_command,password_len+10);
