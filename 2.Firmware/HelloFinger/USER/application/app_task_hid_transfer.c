...
extern SYSTEM_HARDWARE_INFO_t g_sys_hardware_info;
...
static void Key_Func_Exec(void)
{
	switch ((g_key_data_format.cmd) & 0x0F)	//响应码会在原命令码基础上|0x80，为了匹配去除高位
	{
		case CMD_GET_INFO:
		{
			...
			g_sys_hardware_info.CH9329_INFO[0] = version;
			g_sys_hardware_info.CH9329_INFO[1] = connection;
			g_sys_hardware_info.CH9329_INFO[2] = led_state;
	 ...
	 if((g_key_data_format.data[0] == 0xFE) && (result == (uint8_t)OPERATE_SUCCESS))	//判断是否为自定义USB通信协议头
	 {
		 ...
		 switch (g_key_data_format.data[2])		//判断指令类型
		 {
			 ...
			 case USB_PROTOCOL_FORMAT_SET_FINGER_COLOR:	//设置指纹模块颜色
			 {
				 ...
				 FPM383C_ControlBLN((_LED_Function_t)func,start_color,end_color,cycle_time);
				 break;
			 }
			 ...
			 case USB_PROTOCOL_FORMAT_GET_FW:
			 {
				 ...
				 g_usb_response.type = USB_PROTOCOL_FORMAT_GET_FW;
				 ...
				 g_usb_response.data[48] = CH9329_CAL_SUM((uint8_t*)&g_usb_response,48);
				 CH9329_Send_HID_Data((uint8_t*)&g_usb_response,49);
				 break;
			 }
			 case USB_PROTOCOL_FORMAT_GET_HW:
			 {
				 ...
				 g_usb_response.head = USB_RESPONSE_HEAD;
				 g_usb_response.len = 37;
				 g_usb_response.type = USB_PROTOCOL_FORMAT_GET_HW;
				 g_usb_response.result = CONFIRM_OK;
				 
				 memcpy((uint8_t*)&g_usb_response.data[0],&flashId,sizeof(uint16_t));
				 g_usb_response.data[2] = g_sys_hardware_info.CH9329_INFO[0];
				 memcpy((uint8_t*)&g_usb_response.data[3],g_sys_hardware_info.FPM383C_INFO,sizeof(g_sys_hardware_info.FPM383C_INFO));
