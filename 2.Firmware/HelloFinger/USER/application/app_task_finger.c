...
static void vTaskFingerProcessing(void)
{
	...
	FPM383C_GetChipSN();
	xEventGroupSetBits((EventGroupHandle_t)FingerEvent_Handle,(EventBits_t)EVENT_GET_CHIP_SN);
	while(1)
	{
...
static void Finger_Func_Exec(void)
{
	...
	switch (g_data_format.type)	//判断包标识
	{
		case ID_CMD:	//命令包
		{
			break;
		...
		case ID_RESP:	//响应包
		{
			switch(g_data_format.data[0])	//判断确认码
			{
				...
				case CONFIRM_OK:
				{
					...
					if(event_status & EVENT_GET_CHIP_SN)
					{
						for(uint8_t i=0;i<sizeof(g_sys_hardware_info.FPM383C_INFO);i++)
						{
							g_sys_hardware_info.FPM383C_INFO[i] = g_data_format.data[i+1];
						}
						xEventGroupClearBits(FingerEvent_Handle,EVENT_GET_CHIP_SN);
					}
					break;
				}
